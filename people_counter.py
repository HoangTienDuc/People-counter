...
def init_dlib_tracker(rgb, coors, trackers):
    startX, startY, endX, endY = coors
    tracker = dlib.correlation_tracker()
    rect = dlib.rectangle(startX, startY, endX, endY)
    tracker.start_track(rgb, rect)

    trackers.append(tracker)
def dlib_tracker(rgb, rects, trackers, frame):
...
def id_associate_count(frame, ct, rects, trackableObjects, W, H, totalDown, totalUp):
...
def draw_result(frame, totalDown, totalUp, H, videotime):
    # frame
    info = [
        ("Up", totalUp),
        ("Down", totalDown),
...
def counter(filenameOpen, filenameSave):
    ...
    while True:
        ...
        if totalFrames % defaultSkipFrames == 0:
            ...
            for i in np.arange(0, detections.shape[2]):
                ...
                if confidence > defaultConfidence:
                    ...
                    coors = box.astype("int")
                    init_dlib_tracker(rgb, coors, trackers)

        # object *detectors* to obtain a higher frame processing throughput
        
        else:
            dlib_tracker(rgb, rects, trackers, frame)
        ...
        totalDown, totalUp = id_associate_count(frame, ct, rects, trackableObjects, W, H, totalDown, totalUp)

        draw_result(frame, totalDown, totalUp, H, videotime)
